...
CvTileAlgorithmFactory<InputPixelType, InputBandCount, OutputPixelType, OutputBandCount>::makeCvTileAlgorithm (boost::program_options::variables_map& gpu_alg_params, __attribute__((unused)) const size_t cuda_device_id) {
	...
	if (boost::iequals(algorithm,"GpuErode")) {
		...
		GpuErode *erode = new GpuErode(CUDA_DEVICE,roi_width,roi_height,buffer_radius);
		...
		GpuAbs *abs = new GpuAbs(CUDA_DEVICE,tile_width,tile_height);
		...
		GpuDilate *dilate = new GpuDilate(CUDA_DEVICE,roi_width,roi_height,buffer_radius);
		...
		GpuWhs *whs = new GpuWhs(CUDA_DEVICE,roi_width,roi_height,buffer_radius);
